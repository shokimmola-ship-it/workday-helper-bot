...
export function DocumentSidebar({ docs, selected, onToggle, onRemove, onNew, onUpload }: Props) {
  const fileRef = useRef<HTMLInputElement>(null);

  return (
    <aside className="ink-panel flex h-full w-full flex-col rounded-2xl p-5 lg:w-80">
      <div className="flex items-center gap-3">
        <div className="flex size-9 items-center justify-center rounded-xl bg-accent text-accent-foreground font-display text-sm font-bold">
          WD
        </div>
        <div>
          <h1 className="text-base font-semibold">Workdesk</h1>
          <p className="text-xs opacity-70">Document intelligence</p>
        </div>
      </div>
      ...
      <p className="mt-6 text-[11px] font-semibold uppercase tracking-[0.14em] opacity-60">
        ...
        <div className="space-y-1.5 py-1">
          {docs.length === 0 && (
            <p className="rounded-lg border border-dashed border-white/15 p-4 text-xs leading-relaxed opacity-70">
              No documents yet. Paste meeting notes, a spec, or upload a text file to begin.
            </p>
          )}
          {docs.map((doc) => {
      ...
      <p className="mt-4 border-t border-white/10 pt-3 text-[11px] leading-relaxed opacity-55">
        Selected documents are the only context the AI reads. Everything stays in this browser.
      </p>
    </aside>
  );
}